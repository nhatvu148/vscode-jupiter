// Regenerates src/data.ts from a local psj-editor checkout (with the
// jupiter_utils submodule), the authoritative source PSJ scripts import.
//
//   yarn generate --psj-editor ../psj-editor
//   node scripts/generate.mjs --psj-editor <path> [--utils <path to jupiterutils pkg>]
//
// Sources:
//   jupiter_utils/jupiterutils/PSJ_Classes.py  -> 1815 PSJ commands (signatures + docstrings)
//   jupiter_utils/jupiterutils/Utility.py      -> JPT utilities (class JPT)
//   IDEData/Keywords.dat                        -> PSJ utility / GUI keyword groups
//
// For each function we emit an accurate signature (real param names + defaults,
// types inferred from Hungarian prefixes) plus the `## Description` docstring
// when present.
...
const psjEditor =
  flag("psj-editor", process.env.PSJ_EDITOR_DIR) ?? join(REPO, "..", "psj-editor");
const utilsPkg = flag(
  "utils",
  join(psjEditor, "jupiter_utils", "jupiterutils"),
);
const keywordsDat = join(psjEditor, "IDEData", "Keywords.dat");

if (!existsSync(utilsPkg)) {
  console.error(
    `jupiterutils package not found at ${utilsPkg}\n` +
      `Pass --psj-editor <checkout> (with the jupiter_utils submodule), or --utils <path>.`,
  );
  process.exit(1);
}
...
const out =
  ...
  `// Do not edit by hand — run \`yarn generate --psj-editor <path>\` instead.\n` +
